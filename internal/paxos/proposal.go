...
// A proposal number is a unique identifier that totally orders all proposals across all proposers in the system. It's the foundation upon which Paxos builds its safety guarantees.

// Think of it like a "priority ticket" - higher numbers win, and no two proposers can ever have the same number.

// Why proposal numbers exist ?

// Problem: Multiple proposers might try to propose different values at the same time. How do we decide which one "wins"?

// Solution: Each proposal carries a unique number.Acceptors always prefer higher numbers. This creates a total ordering of proposals.

// Key insight: Proposal numbers are not sequence numbers of values. They're competition markers - "my proposal is newer/higher priority than yours".
...
// No two proposal numbers anywhere in the system may be identical. This is achieved by combining a local counter with a globally unique proposer ID.

// Why: If two proposers use the same proposal number, acceptors can't distinguish between them, and both might get accepted with different values, breaking Paxos safety.

// COMMON BUG TO AVOID

// BUG: Using only a counter without proposer ID

// If Proposer-A uses proposal number 5, and Proposer-B also uses 5 and they propose different values, some acceptors might accept A's value while others accept B's value - BOTH at "proposal 5".The system would think the same proposal has two different values.This violates safety.

// FIX: Always include a unique proposer ID as the second component.

// MULTI-PAXOS EXTENSION POINT

// In Single-Decree Paxos, we just have one consensus instance. In Multi-Paxos, we have many slots/instances, each choosing one value.The proposal number structure stays the same - it's about ordering proposals within a single slot. The "slot number" is a separate concept added to messages, not to proposal numbers.
